...
import tn.esprit.services.ReservationActiviteServices;
...
public class ReservationActiviteController {
    ...
    private final ReservationActiviteServices service = new ReservationActiviteServices();
    ...
    private final ObservableList<ReservationActivite> list = FXCollections.observableArrayList();
    private final FilteredList<ReservationActivite> filteredList = new FilteredList<>(list, p -> true);
    ...
    public void initialize() {
        ...
        colActivite.setCellValueFactory(c -> new SimpleStringProperty(activiteDisplay.getOrDefault(c.getValue().getIdActivite(), "")));
    ...
    public void refresh() {
        try {
            ...
            for (Activite a : activiteService.afficher()) activiteDisplay.put(a.getIdActivite(), a.getNom());
            list.clear();
    ...
    @FXML private void onAdd() {
        ...
        if (showDialog(r, "Ajouter réservation activite")) {
            try {
                service.ajouter(r);
                refresh();
    ...
    @FXML private void onEdit() {
        ReservationActivite r = table.getSelectionModel().getSelectedItem();
        if (r == null) { showError("Attention", "Selectionnez une ligne."); return; }
        if (showDialog(r, "Modifier réservation activite")) {
            try {
                service.modifier(r);
                refresh();
    ...
    @FXML private void onDelete() {
        ...
        if (confirm("Supprimer cette réservation ?")) {
            try {
                service.supprimer(r.getIdReservationActivite());
                refresh();
            } catch (SQLException e) { showError("Erreur", "Suppression impossible."); }
        }
    }

    private boolean showDialog(ReservationActivite r, String title) {
        Dialog<ReservationActivite> d = new Dialog<>();
        d.setTitle(title);
        d.initModality(Modality.APPLICATION_MODAL);
        DialogStyleHelper.styleDialogPane(d.getDialogPane());
        ...
        comboActivite.setConverter(new StringConverter<Activite>() {
            @Override public String toString(Activite a) { return a == null ? "" : a.getNom(); }
        ...
        try {
            ...
            comboActivite.getItems().addAll(activiteService.afficher());
        ...
        if (r.getIdActivite() > 0) comboActivite.getItems().stream().filter(a -> a.getIdActivite() == r.getIdActivite()).findFirst().ifPresent(comboActivite.getSelectionModel()::select);
        else if (!comboActivite.getItems().isEmpty()) comboActivite.getSelectionModel().selectFirst();
        GridPane g = DialogStyleHelper.buildGrid();
        ...
        DialogStyleHelper.addRow(g, 5, "Activite *", comboActivite);
        ...
        DialogStyleHelper.styleCombo(comboActivite);
        ...
        d.setResultConverter(btn -> {
            ...
            if (selActivite == null) { showError("Validation", "Activite obligatoire."); return null; }
            ...
            r.setIdActivite(selActivite.getIdActivite());
            return r;
        });
