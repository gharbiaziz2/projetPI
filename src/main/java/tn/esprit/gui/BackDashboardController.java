...
public class BackDashboardController implements Initializable {
    ...
    private boolean alertShown = false; // Prevent showing the alert multiple times per session
    ...
    public void refresh() {
        ...
        checkCriticalVoyages();
    }

    private void checkCriticalVoyages() {
        if (alertShown) return;
        try {
            List<Voyage> voyages = voyageServices.afficher();
            for (Voyage v : voyages) {
                if (v.getDateDepart() != null && v.getPlacesDisponibles() > 0) {
                    long daysBetween = ChronoUnit.DAYS.between(LocalDate.now(), v.getDateDepart());
            ...
            System.err.println("Erreur vérification alertes: " + e.getMessage());
        }
    }
